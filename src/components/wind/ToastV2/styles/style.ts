...
const getBackground = css<StyledToastProps>`
	background-color: ${({ color }) =>
		//@ts-ignore
		ToastColors[color]?.background};
`;

const getColor = css<StyledToastProps>`
	color: ${({ color }) =>
		//@ts-ignore
		ToastColors[color]?.text};
`;

const getBorder = css<StyledToastProps>`
	border: ${({ color }) =>
		//@ts-ignore
...
export const PositionContainer = styled.div<CardProps>`
	z-index: 2;
	position: fixed;
	${(props) =>
		props.position
