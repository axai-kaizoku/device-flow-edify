...
export const StyledText = styled.div`
  ...
  ${({ variant }: TypographyProps) =>
    headerTags.includes(variant) ? getHeaderTag : getTag}
  ${({ isEllipsis }: TypographyProps) => isEllipsis && Ellipsis}
`;
...
const Ellipsis = css`
  ...
  display: -webkit-box;
  ${({ lineClamp }: TypographyProps) =>
    lineClamp &&
    css`
      -webkit-line-clamp: ${({ lineClamp }: TypographyProps) => lineClamp || 1};
    `}
...
const getTag = css`
  color: ${({ color }: TypographyProps) => color || Colors.grey_900};
  ${({ type }: TypographyProps) => getFontFamily};
  ${({ variant }: TypographyProps) => {
    switch (variant) {
      case "body-text1":
        return BodyText1;
...
const getHeaderTag = css`
  color: ${({ color }: TypographyProps) => color || Colors.grey_900};
  ${({ type }: TypographyProps) => (type ? getFontFamily : FontSemiBold)};
  ${({ variant }: TypographyProps) => {
    switch (variant) {
      case "h1":
        return H1;
