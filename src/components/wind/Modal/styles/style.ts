...
export const getModalWidth = css`
...
export const ModalBody = styled.div`
  padding: 32px 24px;
  max-height: ${({ hasFooter }: { hasFooter: boolean | undefined }) =>
    hasFooter ? "400px" : "450px"};
  overflow-y: auto;
  @media screen and (max-width: 600px) {
    padding: 24px 16px;
  }
...
export const ModalFooterWrap = styled.div`
  ...
  padding: 12px 24px;
  justify-content: ${({ inline }: { inline: boolean | undefined }) =>
    inline ? "end" : "center"};
  @media screen and (max-width: 600px) {
    padding: 12px 16px;
  }
