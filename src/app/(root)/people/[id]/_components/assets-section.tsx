            ...
            <div className='flex flex-col mt-5 gap-1'>
                {user?.devices?.slice(0,2).map((device: Device)=>(
                    <div className='flex justify-start items-center gap-4 pb-4 border-b'>
                        <div className=''>
                            <img src={`${device?.image || '/media/mac-2.png'}`} alt="Asset-1" />
                        </div>
                        <div>
                            <div className='font-semibold text-xl'>{device?.device_name}</div>
                            <div className='text-[#7C7C7C] font-medium text-base'>{device?.ram} . {device?.storage}</div>
                ...
                {totalAssets >2 && <div className='text-[#9B9B9B] font-semibold text-lg my-2 text-center'>+{totalAssets -2 } more</div>}

                <button className='text-white bg-black font-semibold text-lg w-full mt-2 py-3 rounded-full' onClick={()=>{
                    router.push('/assets');
