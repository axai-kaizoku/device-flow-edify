		...
		<div className="flex flex-col justify-center items-start pt-12 px-4 space-y-6">
            ...
            <div className='w-full'>
                ...
                <div className='flex flex-col gap-6 w-full mb-4'>
                    {severityArray.map((sev)=>(
                        <div
                            ...
                            //         : 'border-gray-300'
                            // }`}
                            className={`border-2 border-black w-full flex items-center px-2 py-4 text-lg cursor-pointer rounded-lg ${formData.severity === sev ? 'border-black bg-gray-50': 'border-gray-300'}`}
                            onClick={() => changeHandler("severity", sev)}
                        >
                            <input type="radio"
                                name="severity"
                                id={sev}
                            ...
                            <label
                                ...
                                className={`cursor-pointer ${
                                    formData.severity === sev ? 'text-black' : 'text-gray-600'
                ...
                <div>
                    ...
                    <Textarea name='issue_message' value={formData.issue_message} onChange={(event)=>{ changeHandler(event.target.name, event.target.value) }} width="100%" style={{outline:"none", borderRadius:"0.5rem"}} placeholder="So recently, I've been running into an issue with the device..." />
                </div>
