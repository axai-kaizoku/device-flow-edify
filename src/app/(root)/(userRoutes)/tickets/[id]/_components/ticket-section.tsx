...
import ChatInterface from "@/app/(root)/issues/[id]/_components/chat-interface";
    ...
    <div className="bg-white rounded-[10px] w-full border border-[rgba(0, 0, 0, 0.10)]  mt-3 flex h-[100vh] overflow-y-hidden">
      ...
      <div className="w-[40%] h-full border border-r border-l-0 border-t-0 border-b-0 border-[rgba(0, 0, 0, 0.05)] overflow-y-auto">
        <div className="px-6">
          ...
          {data?.deviceDetails && (
            <div
              className="border-[#E5E5E5] border mt-6 p-3 flex justify-between items-center rounded-md w-full cursor-pointer"
              onClick={() => {
                router.push(`/devices/${data?.deviceDetails?._id}`);
              }}
            >
