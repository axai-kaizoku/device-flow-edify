...
export const createSignupLink = cache(async function (teamId: string) {
  try {
    ...
    const res = await callAPIWithToken<{ message: string; link: string }>(
      `https://api.edify.club/edifybackend/v1/auth/generate-temp-link`, // API endpoint
      "POST", // HTTP method
      {
...
export const validateToken = cache(async (token: string) => {
  try {
    const res = await callAPI<ValidationResponse>(
      `https://api.edify.club/edifybackend/v1/auth/validate/temp-link`, // API endpoint
      "POST", // HTTP method
      {
        token: token,
      }
...
export const signupReqOTP = async (email: string) => {
  try {
    const res = await callAPI<{}>(
      `https://api.edify.club/edifybackend/v1/auth/send/email-otp`,
      "POST",
      {
        email: email,
      }
...
export const verifySignupOTP = async (email: string, otp: string) => {
  try {
    const res = await callAPI<{}>(
      `https://api.edify.club/edifybackend/v1/auth/verify/email-otp`,
      "POST",
      {
        email: email,
        otp: otp,
...
export const signupEmployee = async ({
  ...
  try {
    const res = await callAPI<User>(
      `https://api.edify.club/edifybackend/v1/auth/link-register`,
      "POST",
      {
        token,
        first_name,
